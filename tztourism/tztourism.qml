...
Rectangle {
    ...
    Rectangle {
            ...
            Rectangle {
                ...
                Column {
                    ...
                    Rectangle {
                        ...
                        // Single VideoOutput — in fullscreen mode it scales up
                        // to cover the entire safariTvOverlay via transform.
                        // No second surface, no GPU duplication.
                        VideoOutput {
                            id: videoOut
                            anchors.fill: parent
                            ...
                            // Map tvScreen's top-left into safariTvOverlay coords,
                            // then scale videoOut so it fills the full overlay.
                            property real fsScaleX: safariTvOverlay.width  / width
                            property real fsScaleY: safariTvOverlay.height / height

                            // Offset needed to re-center after scale
                            property real fsDx: (safariTvOverlay.width  - width)  / 2
                                                - tvScreen.mapToItem(safariTvOverlay, 0, 0).x
                                                - anchors.margins
                            property real fsDy: (safariTvOverlay.height - height) / 2
                                                - tvScreen.mapToItem(safariTvOverlay, 0, 0).y
                                                - anchors.margins

                            transform: [
                                Translate {
                                    x: safariTvOverlay.tvFullScreen ? videoOut.fsDx : 0
                                    y: safariTvOverlay.tvFullScreen ? videoOut.fsDy : 0
                                ...
                                Scale {
                                    xScale: safariTvOverlay.tvFullScreen ? videoOut.fsScaleX : 1.0
                                    yScale: safariTvOverlay.tvFullScreen ? videoOut.fsScaleY : 1.0
                                ...
                                Rotation {
                                    angle: safariTvOverlay.tvFullScreen ? fsLayer.videoRotation : 0
                                    origin.x: videoOut.width  / 2
                                    origin.y: videoOut.height / 2
                                    Behavior on angle { RotationAnimation { duration: 300; direction: RotationAnimation.Clockwise } }
                                }
                            ]
                        }
        ...
        // The actual video is videoOut (in tvScreen) scaled up via
        // transform — single VideoOutput, zero GPU duplication.
        Item {
            id: fsLayer
            anchors.fill: parent
            visible: safariTvOverlay.tvFullScreen
            ...
            Item {
                ...
                // ── Video is rendered by videoOut in tvScreen ──────
                // (scaled up via transform when fullscreen — no second
                //  VideoOutput needed here)
